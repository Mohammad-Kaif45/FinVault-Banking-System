...
function Transfer() {
  ...
  // 🚀 BULLETPROOF FIX: Forcefully grab the ID directly from the URL (e.g., gets '1' from '/transfer/1')
  ...
  const handleTransfer = async (e) => {
    ...
    try {
      ...
      alert("Transfer Failed: Please verify target account number and balance.");
    }
  };
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "sans-serif", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ flex: "0.4", backgroundColor: theme.navy, color: "white", padding: "60px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
        ...
        <p style={{ color: "#94a3b8", fontSize: "16px", lineHeight: "1.6" }}>Securely dispatch capital across the FinVault network with real-time settlement.</p>
      </div>
      ...
      <div style={{ flex: "0.6", backgroundColor: theme.bg, display: "flex", justifyContent: "center", alignItems: "center", position: "relative" }}>
        ...
        <div className="form-card" style={{ width: "100%", maxWidth: "420px", backgroundColor: theme.white, padding: "48px", borderRadius: "8px", border: `1px solid ${theme.border}` }}>
          ...
          {/* 👇 This will now accurately show the ID 1 or 2 instead of being blank */}
          <p style={{ color: theme.textSec, fontSize: "14px", marginBottom: "32px", textAlign: "center" }}>
            From Account ID: <span style={{color: theme.textMain, fontWeight: "600"}}>{id}</span>
          </p>

          <form onSubmit={handleTransfer}>
            <div style={{ marginBottom: "20px" }}>
              <label style={{ display: "block", fontSize: "13px", fontWeight: "500", color: theme.textMain, marginBottom: "8px" }}>Target Account Number</label>
              <input
                type="text"
            ...
            <button type="submit" style={{ width: "100%", backgroundColor: theme.primary, color: "white", padding: "14px", borderRadius: "6px", border: "none", fontWeight: "600", cursor: "pointer" }}>Authorize Transfer</button>
            <button type="button" onClick={() => navigate("/dashboard")} style={{ width: "100%", backgroundColor: "transparent", color: theme.textSec, padding: "10px", border: "none", cursor: "pointer", marginTop: "10px" }}>Cancel</button>
          </form>
        </div>
      </div>
    </div>
